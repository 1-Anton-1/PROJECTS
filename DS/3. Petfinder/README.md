...
Задачу можно решать как методами классификации, так и методами регрессии. В данном случае создаем модель классфикации.
...
11. Применив метод RandomForest на 1019 фичах, получаем низкие результаты. Слишком много фичей. Применим метод главных компонент и понизим размерность эмбеддинговых фичей с 1000 до 6.
12. Приджойним "сжатые картинки" к нашему датасету из 19 фичей (изначальный датасет).
13. Посмотрим на score модели-экземпляра класса RandomForestClassifier. Score увеличился на несколько процентов!
14. Импортируем CatBoostClassifier и обучим модель catboost на подготовленных данных. Посмотрим на score и сделаем предсказания на текстовом датасете. 
